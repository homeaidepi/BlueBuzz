...
EXCLUDES='--exclude-glob .git --exclude-glob .DS_Store --exclude-glob _sess/*  --exclude-glob uploads/* --exclude-glob deploy.sh'
...
#Do the thing.
lftp -c "set ftp:list-options -a; open ftp://$FTPUSER:$FTPPASS@$FTPHOST; lcd $LOCALPATH; cd $REMOTEPATH; mirror --no-perms --reverse $DELETE --use-cache --verbose --no-umask --parallel=3 $EXCLUDES"